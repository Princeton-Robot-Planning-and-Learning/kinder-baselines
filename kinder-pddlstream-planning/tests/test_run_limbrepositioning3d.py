...
MOVE_BASE_REQUIRED_VARIANT = "wheelchair-right-arm"
...
def test_the_sweep_applies_the_per_variant_overrides():
    ...
    variant."""
    # Asserted against the table rather than a fixed list, which empties as variants
    # stop needing an override.
    for variant, overrides in _VARIANT_OVERRIDES.items():
        merged = variant_kwargs(variant, **dict.fromkeys(overrides, "shared"))
        assert all(merged[name] == value for name, value in overrides.items())
...
def test_the_shipped_base_placement_cannot_reach_the_goal():
    ...
    Checked on the scene's own transform. The default grasp sits further up the limb,
    which pulls the target toward the robot and does reach this goal.
    """
    ...
    try:
        ...
        grasp = sim.scene.grasp_transform
        release_grasp(sim)
...
def test_controllability_is_measured_at_the_ik_solution(limb_env):
    """Regression test: the gate must read the pose under test, not the IK seed."""
    sim, ctx = limb_env
    init_conf, _ = _confs(ctx)
    (grasp,) = next(sample_grasp(ctx, LIMB_NAME))
    release_grasp(sim)
    try:
        sim.robot.set_base(ctx.grasp_base_pose)
        ...
        left_behind = list(sim.robot.arm.get_joint_positions())
        assert not np.allclose(
            left_behind[:NUM_ROBOT_JOINTS], solution[:NUM_ROBOT_JOINTS]
        )

        # And the distinction is not cosmetic: the hold differs between the two.
        hold_where_left = limb_hold_torque(sim)
        sim.robot.arm.set_joints(list(solution))
        hold_at_solution = limb_hold_torque(sim)
        assert not np.allclose(hold_where_left, hold_at_solution)

        # Passing the solution places the arm there before anything is measured.
        sim.robot.arm.set_joints(left_behind)
        limb_is_controllable(ctx, solution)
        assert np.allclose(
            list(sim.robot.arm.get_joint_positions())[:NUM_ROBOT_JOINTS],
            solution[:NUM_ROBOT_JOINTS],
        )
    finally:
        reset_to_start(ctx)
...
def test_state_restore_round_trips_through_the_grasp(limb_env):
    ...
    # Loose enough for the contact solver's own noise, tight enough that a state the
    # restore failed to carry would show up.
    for field in ("robot_positions", "limb_positions", "limb_velocities"):
        assert np.allclose(getattr(first, field), getattr(second, field), atol=1e-6)
    reset_to_start(ctx)
...
def test_mpc_makes_progress_toward_the_goal(limb_env):
    ...
    for _ in range(25):
        committed = mpc.step()
        assert len(committed) == ctx.mpc.commit_steps
        for torque in committed:
...
def test_the_directed_torque_drives_the_limb_toward_the_goal(limb_env):
    """A positive gain moves the limb in, and the candidate set carries one per gain."""
    sim, ctx = limb_env
    init_conf, goal_conf = _confs(ctx)
    (grasp,) = next(sample_grasp(ctx, LIMB_NAME))
    _, _, state = next(plan_grasp_motion(ctx, LIMB_NAME, grasp, init_conf, goal_conf))
    ...
    ctx.mpc = MPCConfig()
    restore_state(sim, state)
    start = capture_state(sim)
    errors = {}
    for gain in (ctx.mpc.directed_gains[0], 0.0):
        restore_state(sim, start, regrasp=False)
        torque = list(
            np.clip(limb_directed_torque(sim, goal, gain), *ctx.robot_torque_limits)
        )
        for _ in range(ctx.mpc.horizon * ctx.mpc.action_repeat):
            advance_corrected(sim, torque)
        errors[gain] = float(
            np.abs(np.subtract(sim.limb.get_joint_positions(), goal)).sum()
        )
    assert errors[ctx.mpc.directed_gains[0]] < errors[0.0]

    restore_state(sim, start, regrasp=False)
    mpc = PredictiveSamplingMPC(ctx, goal)
    candidates = mpc._sample_candidates(1.0)  # pylint: disable=protected-access
    assert len(candidates) == ctx.mpc.num_rollouts
    directed = candidates[2 : 2 + 2 * len(ctx.mpc.directed_gains)]
    assert all(np.allclose(c, c[0]) for c in directed[::2])
    reset_to_start(ctx)


def test_rollouts_that_break_a_limit_are_rejected_not_penalised(limb_env):
    """A violating rollout sorts behind every clean one, and the penalties still
    rank."""
    sim, ctx = limb_env
    init_conf, goal_conf = _confs(ctx)
    (grasp,) = next(sample_grasp(ctx, LIMB_NAME))
    ...
    mpc = PredictiveSamplingMPC(ctx, goal)
    start = capture_state(sim)
    # A torque far outside the robot's limits violates on the very first step.
    huge = np.full((ctx.mpc.num_control_points, NUM_ROBOT_JOINTS), 1e4)
    rejected = mpc._score(start, mpc._expand(huge))  # pylint: disable=protected-access
    assert rejected[0] == 1
    assert rejected[1] >= MPCConfig().robot_torque_violation_weight

    clean = mpc._score(  # pylint: disable=protected-access
        start, mpc._expand(np.zeros_like(huge))  # pylint: disable=protected-access
    )
    assert clean[0] == 0 and clean < rejected

    ctx.mpc = MPCConfig(num_rollouts=8, reject_violations=False)
    soft = PredictiveSamplingMPC(ctx, goal)
    assert (
        soft._score(start, soft._expand(huge))[0]  # pylint: disable=protected-access
        == 0
    )
    reset_to_start(ctx)
...
def test_torques_stay_inside_the_environment_limits(limb_env):
    ...
    for _ in range(5):
        torques = np.asarray(mpc.step())
        assert (torques >= lower - 1e-9).all() and (torques <= upper + 1e-9).all()
    reset_to_start(ctx)
...
def test_the_base_may_not_be_parked_in_the_person(leg_env):
    ...
    `base_in_collision` used to consult the empty `scene_collision_ids` alone.
    """
    sim, ctx = leg_env
    try:
        assert not base_in_collision(ctx), "the start pose is clear"
        ...
        for body_id in (sim.scene.torso_id, sim.limb.robot_id):
            ...
            # The base sits metres below the person on the leg variants, so park it at
            # the body's own height or nothing can ever overlap.
            sim.robot.base.z = position[2]
            sim.robot.set_base(
                SE2Pose(position[0], position[1], ctx.grasp_base_pose.rot)
            )
            assert base_in_collision(ctx), f"base parked inside body {body_id}"
    finally:
        reset_to_start(ctx)
